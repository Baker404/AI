...
class WebScraping():
    ...
    # construct the soup parser
        ...
        # finally, if the url is valid
            if WebScraping.is_valid(img_url):
                urls.append(img_url)
    ...
    def main(url, path):
    # get all images
        imgs = WebScraping.get_all_images(url)
        for img in imgs:
        # for each img, download it
            WebScraping.download(img, path)
if __name__ == "__main__":
    #path = "D:\wiu\cradu\image downloder"
    url = 'URL'
    WebScraping()
    WebScraping().is_valid("")
    WebScraping().get_all_images("")
    WebScraping().download("","")
    WebScraping().main("","")